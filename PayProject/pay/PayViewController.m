...
@interface PayViewController ()<AVCaptureMetadataOutputObjectsDelegate,UIAlertViewDelegate>
...
- (void)customUI{
    ...
    [_PhotoButton setImage:[UIImage imageNamed:@"cameraBtn"] forState: UIControlStateNormal];
    [_PhotoButton setImage:[UIImage imageNamed:@"cameraBtn"] forState:UIControlStateNormal];
...
//    }
    ...
    NSLog(@"宽：%f  高：%f",self.previewLayer.frame.size.width,self.previewLayer.frame.size.height);
    //开始启动
...
//    self.videoConnection = [AVCaptureConnection  OutputconnectionWithMediaType:AVMediaTypeVideo];
...
//        if (is_iOS8) {
//            self.previewLayer.nection.videoOrientation con= AVCaptureVideoOrientationPortrait;
//        }else{
